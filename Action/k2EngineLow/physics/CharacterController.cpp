...
namespace nsK2EngineLow {
	...
	const Vector3& CharacterController::Execute(Vector3& moveSpeed, float deltaTime)
	{
		...
		//下方向を調べる。
		{
			...
			const float centerOffsetY = m_height * 0.5f + m_radius;
			//始点はカプセルコライダーの中心。
			Vector3 sweepStartPos = m_position;
			sweepStartPos.y += centerOffsetY;
			//終点は地面上にいない場合は1m下を見る。
			//地面上にいなくてジャンプで上昇中の場合は上昇量の0.01倍下を見る。
			//地面上にいなくて降下中の場合はそのまま落下先を調べる。
			Vector3 endPos = sweepStartPos;

			if (m_isOnGround == false) {
				if (addPos.y > 0.0f) {
					//ジャンプ中とかで上昇中。
					//上昇中でもXZに移動した結果めり込んでいる可能性があるので下を調べる。
					sweepStartPos = nextPosition;
					sweepStartPos.y += centerOffsetY;
					endPos = sweepStartPos;
			...
			start.setOrigin(btVector3(sweepStartPos.x, sweepStartPos.y, sweepStartPos.z));
