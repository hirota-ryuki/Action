...
namespace nsK2EngineLow {
	...
	public:
		...
		/// <param name="time">アニメーション時間(秒)。</param>
		void PlayAtTime(int clipNo, float time)
		{
			if (clipNo < 0 || clipNo >= static_cast<int>(m_animationClips.size())) {
				return;
			}

			m_numAnimationPlayController = 1;
			m_startAnimationPlayController = 0;

			const int index = GetLastAnimationControllerIndex();
			m_animationPlayController[index].ChangeAnimationClip(m_animationClips[clipNo]);
			m_animationPlayController[index].SetInterpolateTime(0.0f);
			m_animationPlayController[index].SetTime(time);
			m_interpolateTime = 1.0f;
			m_interpolateTimeEnd = 0.0f;
		}
		/// <summary>
